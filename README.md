...
| `component_search` | Search ScriptComponent descendants — filter by category (character, vehicle, weapon, damage, inventory, ai, ui, etc.) and event handlers. Pass `entityType` (e.g. `GenericEntity`, `SCR_ChimeraCharacter`, `Vehicle`) instead to get the components typically attached to that entity type, ranked by co-occurrence count — mined from base-game `.et` prefabs (loose + `.pak`) and cached on disk (mtime-invalidated) to avoid rescanning; helps avoid incompatible-component mistakes |
