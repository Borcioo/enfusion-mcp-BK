...
| `mod_plan` | Structured mod plan (`MODPLAN.json`) as the cross-session handoff — actions: `init`, `read`/`status` (plan + progress summary), `update` (mark a phase/task status, add notes), `next` (next pending phase's tasks). Scopes to an addon via `modName`; supports `dryRun` |
