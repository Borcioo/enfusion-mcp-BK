...
| `component_search` | Search ScriptComponent descendants — filter by category (character, vehicle, weapon, damage, inventory, ai, ui, etc.) and event handlers. Pass `entityType` (e.g. `GenericEntity`, `SCR_ChimeraCharacter`, `Vehicle`) instead to get the components typically attached to that entity type, ranked by co-occurrence count — mined from base-game `.et` prefabs (loose + `.pak`) and cached on disk (mtime-invalidated) to avoid rescanning; helps avoid incompatible-component mistakes. **Caveat**: only components declared *directly* in each `.et` file's own `components { ... }` block are counted — components inherited from a parent prefab (`EntityClass : "{GUID}Parent.et" { ... }`) are not resolved, so this is a "commonly declared on" signal rather than an exhaustive component list |
...
| `project_write` | Write or update project files (supports `dryRun` to preview without writing). Accepts `modName` to scope into a specific addon in a multi-mod workspace. Writing a `.c` script runs a lightweight, non-blocking cross-reference check against the offline API index — hallucinated method calls on known classes surface as inline warnings (with "did you mean" suggestions) without ever blocking the write; classes declared locally in the same file are excluded to avoid false positives |
| `project_patch` | Apply diff-style find-and-replace edits to a project file without rewriting the whole thing — mirrors Claude Code's Edit tool semantics (`oldString` must match exactly once unless `replaceAll` is set; all edits are atomic; supports `dryRun` to preview). Same non-blocking cross-reference check as `project_write` runs on `.c` files after patching |
| `mod_create` | Scaffold a complete addon with directory structure and `.gproj` (supports `dryRun` to preview without writing) |
| `mod_plan` | Structured mod plan (`MODPLAN.json`) as the cross-session handoff — actions: `init`, `read` (plan + progress summary), `update` (mark a phase/task status, add notes), `next` (next pending phase's tasks). Scopes to an addon via `modName`; supports `dryRun` |
